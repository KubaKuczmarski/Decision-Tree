...
class Tree:
    ...
    def train(
        ...
        self.leafs = 0
        examples, t_examples = train_test_split(all_examples, test_size=0.3, random_state=2, shuffle=True)
        classes, t_classes = train_test_split(all_classes, test_size=0.3, random_state=2, shuffle=True)
    ...
    def evaluate(self, examples: np.ndarray, classes: np.ndarray):
        classes_number = np.unique(classes)
        ...
        self.metrics(classes, predicted_classes, classes_number)

    def metrics(self, classes, predicted_classes, classes_number):
...
dataset_path1 = "PlayTennis.csv"
dataset_path2 = "breast-cancer.csv"
dataset_path3 = "CustomerSegmentationDataset.csv"
dataset = pd.read_csv(dataset_path3)
tree = Tree()
tree.train(dataset, class_idx=-1, id_idx=0) # Dla PlayTennis.csv i CustomerSegmentationDataset.csv -  id_idx=0, dla breast-cancer.csv - id_idx=None
